...
export function addNode(
  ...
  if (targetLineIndex !== -1) {
    ...
    if (nodeType === NODE_TYPES.ROUTER) {
      ...
      console.log(conditionline1, conditionline2);
      lines!.splice(targetLineIndex, 0, conditionline1, conditionline2);
    } else {
      targetLineIndex = lines.findIndex((line) => line.id === currNodeId);
      const targetLine = lines[targetLineIndex];
...
export function deleteNode(
  ...
  if (currNodeIndex !== -1) {
    ...
    const otherConditionLines = getTargetDstNodeIdLines(lines, routerNodeId);
    ...
    console.log("getUnrelatedNodes: ", a, b);
    // setCurrLines([...b]);
    // setCurrNodes([...a]);
    onNodesChange([...a]);
    onLinesChange([...b]);
...
export function addBranchNode(
  ...
  console.log(routerId, sameLevelBranchNodes);
  const maxPriority =
